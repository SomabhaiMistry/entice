import 'package:flutter/material.dart';
import 'package:flutter/cupertino.dart';

const String helvetica_neu_bold = "Helvetica Neu Bold";
const String helveticaNeueNeue_medium = "HelveticaNeueNeue Medium";
const String helveticaNeueNeue_regular = "HelveticaNeueNeue Regular";
const String sf_pro_display_semibold = "sfproDisplaySemiBold";
const String sf_pro_display_medium = "sfproDisplayMedium";
const String sfpro_display_bold = "sfproDisplayBold";
const String roboto_regular = "robotoRegular ";

const String chewy_regular = "chewy_Regular";
const String poppins_regular = "poppinsRegular";
const String poppins_bold = "poppinsBold";
const String aladin_regular = "Aladin Regular";
const String fredokaOne_regular = "FredokaOneRegular";
const String chelseaMarket_regular = "ChelseaMarketRegular";